...
public class SimpleFSM_far02 : FSM
{
    //枚举需要实现的状态
    ...
    {
        EffectPoint2 = this.transform.GetChild(0).GetComponent<Transform>();
        ea2 = GetComponent<EnemyAttribute2>();
    ...
    void diaoluo()
    {
        ...
        {
            IASManager.CreateItem(3203, -1, i);
            firstDiaoluo = true;
        }
        if (a == 3 && firstDiaoluo == false)
        {
            IASManager.CreateItem(3104, -1, i);
            firstDiaoluo = true;
        }
    }
    IEnumerator die()
    ...
    {
        var instance = Instantiate(Effect, EffectPoint2.position, EffectPoint2.rotation);
        Destroy(instance, 1f);
    }
